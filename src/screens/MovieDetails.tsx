...
const MovieDetails: React.FC<MovieDetailsProps> = ({ route, navigation }) => {
  ...
  if (!profileContext || !showToast) return null;

  const { toggleWatchlist, checkInWatchlist, continueWatching } = profileContext;
    ...
    <View style={gStyle.container}>
      ...
      >
        ...
        <View style={styles.backdropContainer}>
          <Image
            source={images[movie.backdrop] || images.developerPoster}
        ...
        <View style={styles.infoContainer}>
          <Image
            source={images[movie.poster] || images.developerPoster}
            ...
            <View style={styles.genresContainer}>
              {movie.genres && movie.genres.map((genre: string) => (
                <View key={genre} style={styles.genreChip}>
                  <Text style={genreText}>{genre}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View style={styles.actionsContainer}>
          <PaperButton
            ...
            theme={{ colors: { primary: colors.storageBlue } }}
          >
            {playLabel}
          </PaperButton>

          <PaperButton
            mode={isMovieInWatchlist ? 'contained' : 'outlined'}
            ...
            contentStyle={{ height: 46 }}
            labelStyle={styles.watchlistButtonText}
            textColor={colors.white}
            theme={{
              colors: {
                primary: colors.profileEditBackground,
                outline: colors.categoryBorder
              }
            }}
          >
