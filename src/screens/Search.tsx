...
import { searchMovies, searchSuggestions } from '../services/searchService';
...
const Search: React.FC = () => {
  ...
  if (!showToast || !context) return null;

  const { recentSearches, addSearchTerm, removeRecentSearch, clearAllRecent } = context;
  ...
  const handleSuggestionPress = async (term: string) => {
    ...
    try {
      const movie = await fetchMovieByTitle(term);
      if (movie) {
  ...
  const loadMoreResults = () => {
    ...
    searchMovies(searchText, nextPage)
      .then((res) => {
        if (res.length < 9) {
          setHasMore(false);
        }
